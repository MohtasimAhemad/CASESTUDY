...
ngOnInit(): void {
  ...
  fetchShippingAddress() {
    // Fetch the shipping address from the server or local storage
    const addressData = localStorage.getItem('shippingAddress');
    if (addressData) {
      this.shippingAddress = JSON.parse(addressData);
    }
  }
  calculateTotalAmount() {
    this.totalPrice = this.cartList.reduce((total, item) => total + item.quantity * item.price, 0);
  }
  getWalletBalance(){
